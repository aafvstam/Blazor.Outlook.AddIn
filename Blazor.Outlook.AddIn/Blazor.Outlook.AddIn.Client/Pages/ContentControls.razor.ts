...
export async function setupDocument() {
    ...
    await Outlook.run(async (context) => {
        ...
        context.document.body.paragraphs
            .getLast()
            .insertText(
                "With Outlook add-ins, you can use familiar web technologies such as HTML, CSS, and JavaScript to build a solution that can run in Outlook across multiple platforms, including on the web, Windows, Mac, and iPad. Learn how to build, test, debug, and publish Outlook add-ins.",
                "Replace"
            );
    });
}
 ...
 * @param  {} args
 */
async function handleContentControlDeleted(args:any) {
 ...
 * @param  {} args
 */
async function handleSelectionChanged(args:any) {
...
export async function deregisterEvents() {
    await Outlook.run(async (context) => {
        for (let i = 0; i < eventContexts.length; i++) {
            await Outlook.run(eventContexts[i].context, async (context) => {
