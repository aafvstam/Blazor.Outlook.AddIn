...
console.log("Loading Blazor.Outlook.Addin.Client.lib.module.js");
...
export async function beforeWebAssemblyStart(options: any, extensions: any) {
  ...
  Office.onReady((info) => {
    // Check that we loaded into Outlook.
    if (info.host === Office.HostType.Outlook) {
      console.log("We are now hosting in Outlook.");

      // Set the startup behavior of the add-in so that it loads whenever the Outlook document is loaded.
...
/**
 * Handle the changed event from the Outlook document.
 *
 * @param event The event information from Outlook
 */
