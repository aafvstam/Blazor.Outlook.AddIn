 ...
 * @param event
 */
async function insertTextInOutlook(event) {

    console.log("In insertTextInOutlook");
    ...
    try {
        await Outlook.run(async (context) => {
            ...
            textBox.lineFormat.dashStyle = Outlook.ShapeLineDashStyle.solid;

            // Align text in the middle of the text box
            textBox.textFrame.textRange.paragraphFormat.horizontalAlignment = Outlook.ParagraphHorizontalAlignment.center;
        ...
        console.log("Finish insertTextInOutlook");
    }
...
/**
 * Writes the text from the Home Blazor Page to the Outlook slide
 * @param {any} event
 */
async function callBlazorOnHome(event) {

    // Implement your custom code here. The following code is a simple Outlook example.  
    try {
...
/**
 * Writes the text from the Counter Blazor Page to the Outlook slide
 * @param {any} event
 */
 ...
 * The string is then inserted into a Outlook slide as a text box.
    ...
    try {
        ...
        try {
            ...
            if (dotnetloaded === true) {
                ...
                name = await DotNet.invokeMethodAsync("Blazor.Outlook.AddIn.Client", methodname, "Blazor Fan");
            }
            else {
    ...
    try {
        ...
        while (result === "" && attempts < 5) {
            try {
                ...
                result = await DotNet.invokeMethodAsync("Blazor.Outlook.AddIn.Client", "PreloaderDummy");
            } catch (error) {
                console.error("Error during DotNet invocation: " + error.message);
            }
            attempts++;
...
Office.actions.associate("insertTextInOutlook", insertTextInOutlook);
